"""Phase 3 -- FROZEN transfer test on LI-Small(-ish): does the recipe generalize,
or did it learn HI-Small's generator?
...
The data: qubit420/ibm-aml-LI-smaller (Hugging Face, no auth), inspected
2026-08-07: same 11-column schema, Is Laundering present, 3,462,024 rows,
595,898 accounts, 2,790 positive accounts = 0.4682% (HI-Small: 1.2342%).
Account overlap with HI-Small: 3 of 595,898 -- genuinely different run.
PROVENANCE CAVEAT, carried on every number: true LI-Small is ~6.9M rows, so
this mirror is likely a ~half subsample; the graph is thinner than IBM shipped.
...
FILES = [os.path.join(DATA, "li_smaller_train.parquet"),
         os.path.join(DATA, "li_smaller_test.parquet")]
...
print("=" * 70)
print("LOAD (uploader's train/test concatenated -- we apply our own protocol)")
...
print()
print("Provenance caveat on every number above: this mirror is ~3.46M of true")
print("LI-Small's ~6.9M rows -- likely a subsample; the graph is thinner than")
print("IBM shipped. A clean LI-Small rerun needs the Kaggle original.")
